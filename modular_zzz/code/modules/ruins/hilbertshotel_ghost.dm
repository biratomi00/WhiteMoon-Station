/obj/item/hilbertshotel/ghostdojo/ui_interact(mob/user, list/modifiers)
	if((SSgamemode.storyteller.name != "Extended (Very Low Chaos)" && user.mind?.antag_datums) || (SSgamemode.storyteller.name != "Extended (Very Low Chaos)" && (HAS_MIND_TRAIT(user, TRAIT_MINDSHIELD))))
